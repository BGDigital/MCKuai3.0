 ...
 * 
 */
package com.mckuai.com.mckuai.encryption;
	...
	public final static byte[] _decode(byte[] byteData) throws IllegalArgumentException
	{
		...
		for (iSrcIdx = 0; iSrcIdx < reviSrcIdx; iSrcIdx++)
		{
			if (byteData[iSrcIdx] == '+')
				...
				if (byteData[iSrcIdx] == '/')
					...
					if (byteData[iSrcIdx] < '0' + 10)
						byteTemp[iSrcIdx] = (byte) (byteData[iSrcIdx] + 52 - '0');
					else
						if (byteData[iSrcIdx] < ('A' + 26))
		...
		if (iSrcIdx < reviSrcIdx)
		{
			if (iSrcIdx < reviSrcIdx - 2)
			{
				...
				/*
				 * wont have just one input byte left (unless input wasn't
				 * base64 encoded ) due to the for loop steps and array sizes,
				 * after "=" pad removed, but for compleatness
				 */
				else
				{
